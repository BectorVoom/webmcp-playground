...
Structured errors have to cross a boundary that speaks only promises and strings.
`ToolBoundaryError` carries the tag across a rejection, so a host that preserves the rejection value
yields the precise error; a host that flattens it yields `ToolExecutionError` with the host's
verbatim message. Which happened is recorded rather than hidden — that fidelity gap is itself a
WebMCP finding, and the conformance suite exercises both paths.
...
`TurnStarted`, `ModelRequested`, `ModelResponded`, `ToolCallParseFailed`, `ToolCallStarted`,
`ToolCallCompleted`, `ToolCallFailed`, `TurnCompleted`, `TurnFailed`, `TurnCancelled`,
`FaultInjected`, `LogRecord`, `Defect`.

Storage is a bounded ring buffer of 5 000 with a discard marker (N2). The inspector renders the most
recent 300 matching events with a "show older" control and an explicit "showing N of M" count —
windowing rather than virtualisation, which meets N2 with no dependency and stays honest about what
is on screen. See [tech-debt.md](../../tech-debt.md) for when that should change.
...
line, so client trace and server log join on one key (R5.8, R8.4). Bound to `127.0.0.1` with CORS
limited to the dev origin (R8.6).
...
- `ToolHostPort` exposes `subscribeToChanges` rather than a `Stream` (§4.1).
...
- The inspector windows rather than virtualises (§5.1, §7).
