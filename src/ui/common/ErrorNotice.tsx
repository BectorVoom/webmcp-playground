...
export function ErrorNotice({
  ...
  return (
    <div data-testid={testId} className={`rounded border px-2 py-1.5 text-ui ${tone}`}>
      <div className="font-mono font-semibold" data-testid={`${testId}-tag`}>
        {tag}
      </div>
      <div className="mt-0.5 text-body text-ink">{message}</div>
      {remedy !== undefined && (
        <div className="mt-1 text-body text-ink-muted" data-testid={`${testId}-remedy`}>
          → {remedy}
        </div>
      )}
      {ids.length > 0 && (
        <div className="mt-1 font-mono text-meta text-ink-muted">{ids.join(' · ')}</div>
      )}
    </div>
  )
}
